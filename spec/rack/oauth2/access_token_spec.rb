  ...
  [:get, :delete, :post, :put].each do |method|
    describe method do
      it 'should delegate to HTTPClient with Authenticator filter' do
        ...
        expect(token.httpclient.request_filter.last).to be_a Rack::OAuth2::AccessToken::Authenticator
    ...
    context 'in debug mode' do
      it do
        Rack::OAuth2.debug do
          expect(token.httpclient.request_filter[-2]).to be_a Rack::OAuth2::AccessToken::Authenticator
          expect(token.httpclient.request_filter.last).to be_a Rack::OAuth2::Debugger::RequestFilter
        end
      end
    end
  end
